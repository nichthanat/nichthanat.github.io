...
const IMG = {
  frontend: 'FIND YOUR VIBE/Front-end.png',
  backend: 'FIND YOUR VIBE/Back-end.png',
  fullstack: 'FIND YOUR VIBE/Full-stack.png',
  mobile: 'FIND YOUR VIBE/Mobile.png',
  data: 'FIND YOUR VIBE/Data.png',
  devops: 'FIND YOUR VIBE/DevOps.png',
};
...
const Q = [
  {
    icon: '🧩', t: 'ถ้าโค้ดของคุณรันไม่ออก คุณจะ…', o: [
      ...
      ['E. ปล่อยโค้ดนั้นไว้ก่อน ไปกินข้าวก่อยกลับมาดู', { mobile: 1 }],
  ...
  },
  {
    icon: '⚡', t: 'ถ้ามีเวลา 1 วันให้สร้างโปรเจกต์อะไรก็ได้ คุณจะทำ…', o: [
      ...
      ['C. แอปมือถือที่ใช้งานง่าย', { mobile: 1 }],
...
const RESULT_TEXT = {
  ...
  data: { title: 'คุณเหมาะกับสาย Data' },
  devops: { title: 'คุณเหมาะกับสาย DevOps' },
};

// ========= State =========
let idx = 0; // current question index
const answers = Array(Q.length).fill(null); // store chosen option index 0..5
let result = null; // เก็บผลลัพธ์หลังจากคำนวณ
...
function compute() {
  ...
  // เก็บผลลัพธ์ไว้ใช้กับปุ่ม Share
  result = k1;
...
// ========= ปุ่ม Share (แก้ไขแล้ว) =========
document.getElementById('btnShare').onclick = () => {
  // URL mapping สำหรับแต่ละ dev path
  const urlMap = {
    backend: 'https://www.google.com/search?q=back-end+developer+%E0%B8%84%E0%B8%B7%E0%B8%AD',
    fullstack: 'https://www.google.com/search?q=Full+Stack+%E0%B8%84%E0%B8%B7%E0%B8%AD',
    frontend: 'https://www.google.com/search?q=front-end+developer+%E0%B8%84%E0%B8%B7%E0%B8%AD',
    mobile: 'https://www.google.com/search?q=dev+mobile+%E0%B8%84%E0%B8%B7%E0%B8%AD',
    data: 'https://www.google.com/search?q=data+scientist+%E0%B8%84%E0%B8%B7%E0%B8%AD',
    devops: 'https://www.google.com/search?q=devops+%E0%B8%84%E0%B8%B7%E0%B8%AD'
  };

  // ตรวจสอบว่ามีผลลัพธ์และ URL ที่ตรงกัน
  if (result && urlMap[result]) {
    playPop(); // เสียง UI feedback
    window.open(urlMap[result], '_blank'); // เปิดในแท็บใหม่
  } else {
    alert('ยังไม่มีผลลัพธ์ กรุณาทำแบบทดสอบให้เสร็จก่อน');
  }
};
